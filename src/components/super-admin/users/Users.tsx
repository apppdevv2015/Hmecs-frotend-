...
const ALL_STATUS = "All Subscriptions";
...
const apiRoleMap: Record<string, string> = {
  "Sub Super Admin": "sub_super_admin",
  Admin: "admin",
  Artisans: "Artisans",
  Supervisor: "Supervisor",
  Operator: "Operator",
...
const formatLastLogin = (
  ...
  const dateStr = loginDate || updatedAt;
  if (!dateStr) return "Never";
  const d = new Date(dateStr);
  if (isNaN(d.getTime())) return "Never";

  if (!loginDate && createdAt) {
    const c = new Date(createdAt);
    if (!isNaN(c.getTime()) && Math.abs(d.getTime() - c.getTime()) < 1000) {
      return "Never";
    }
  }
...
const formatRole = (role?: string) => {
  ...
  if (normalized === "sub_super_admin" || normalized === "sub super admin")
    return "Sub Super Admin";
...
const getRoleClass = (role: string) => {
  switch (role) {
    ...
    case "Sub Super Admin":
      return "bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400 font-semibold";
    case "Admin":
...
function FormField({
  ...
  const baseClass =
    "h-[44px] w-full rounded-2xl border px-4 text-sm outline-none transition focus:border-blue-500 dark:text-white " +
    (error
      ? "border-red-400 dark:border-red-500 "
      : "border-slate-200 dark:border-slate-700 ") +
    (isDisabled
      ? "cursor-not-allowed bg-slate-100/90 text-slate-500 dark:bg-slate-800/80 dark:text-slate-400 opacity-75 select-none"
...
export default function Users() {
  ...
  const roleOptions = useMemo(() => {
    ...
    if (dbRoles.length > 0) {
      dbRoles.forEach((r) => {
        const formatted = formatRole(r.name);
        if (
          formatted !== "Super Admin" &&
          formatted !== "Sub Super Admin" &&
      ...
      allUsersForFilters.forEach((u) => {
        if (
          u.role &&
          u.role !== "Super Admin" &&
          u.role !== "Sub Super Admin" &&
          !list.includes(u.role)
        ) {
          list.push(u.role);
        }
      });
  ...
  const limitNumber =
    pageSize === "all" ? totalFilteredUsers || 1 : Number(pageSize);
  const startItem =
  ...
  const endItem =
    pageSize === "all"
      ? totalFilteredUsers
      : Math.min(currentPage * limitNumber, totalFilteredUsers);
  const filteredTotalPages =
    pageSize === "all" ? 1 : Math.ceil(totalFilteredUsers / limitNumber);
  ...
  const openDeleteModal = (userOrId: User | string | number) => {
    if (typeof userOrId === "object" && userOrId !== null) {
      setDeleteUser(userOrId as ExtendedUser);
      return;
    }
    const selected =
      allUsersForFilters.find((u) => String(u.id) === String(userOrId)) ||
      allUsers.find((u) => String(u.id) === String(userOrId));
...
function UsersTable({
  ...
  return (
    <div className="w-full max-h-[620px] overflow-x-auto overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
      <table className="w-full min-w-[1100px] text-left">
        <thead className="sticky top-0 z-10 border-b border-slate-200 bg-slate-50 shadow-sm dark:border-slate-800 dark:bg-[#081028]">
          <tr>
            {[
              "S.No",
              "User",
    ...
    >
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <FormField
          label={
            mode === "edit"
              ? "Email Address (Unique — Read Only)"
              : "Email Address"
          }
          type="email"
            ...
            {watch("role") === "Sub Super Admin" || watch("role") === "sub_super_admin" || (watch("role") || "").toString().toLowerCase().includes("sub super") || (watch("role") || "").toString().toLowerCase().includes("sub_super") ? (
              <div>
                ...
                <input
                  ...
                  className="h-[48px] w-full cursor-not-allowed select-none rounded-2xl border border-slate-200 bg-slate-100/90 px-4 text-sm font-medium text-slate-500 opacity-75 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-400"
                />
              </div>
            ) : (
              <AppSelect
