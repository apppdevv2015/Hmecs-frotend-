    ...
    <div className="rounded-[2rem] border border-blue-100 bg-white p-6 shadow-sm transition-all dark:border-slate-700 dark:bg-[#0F172A]">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className={`mt-2 text-3xl font-black ${alert.text}`}>{alert.count}</h3>
              </div>

              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${alert.bg}`}>
      ...
      <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50/60 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
        <p className="text-xs font-semibold text-slate-600 dark:text-slate-300">
          Total Active Alerts:
          <span className="ml-2 font-black text-blue-600 dark:text-blue-400">6 Alerts</span>
        </p>
      </div>
    </div>
  );
