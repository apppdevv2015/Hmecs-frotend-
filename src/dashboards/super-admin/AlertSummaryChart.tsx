...
export default function AlertSummaryChart() {
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      try {
        setLoading(true);
        const res = await superAdminMachineService.getDashboardAlertsSummary();
        if (isMounted) {
  ...
  const totalCritical = data.reduce((sum, item) => sum + (item.critical || 0), 0);
  const totalWarnings = data.reduce((sum, item) => sum + (item.warning || 0), 0);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03] xl:col-span-1">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Alert Summary</h2>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        <div className="rounded-xl bg-gray-50 p-3 dark:bg-white/[0.04]">
          ...
          <h3 className="mt-1 text-lg font-bold text-gray-900 dark:text-white">{totalAlerts}</h3>
        </div>

        <div className="rounded-xl bg-red-50 p-3 dark:bg-red-500/10">
          <p className="text-xs text-gray-500 dark:text-gray-400">Critical</p>
          <h3 className="mt-1 text-lg font-bold text-red-500">{totalCritical}</h3>
        </div>
        ...
        <div className="rounded-xl bg-orange-50 p-3 dark:bg-orange-500/10">
          ...
          <h3 className="mt-1 text-lg font-bold text-orange-500">{totalWarnings}</h3>
        </div>
      </div>
    </div>
  );
