...
const roleColor: Record<string, string> = {
  super_admin: "bg-purple-50 text-purple-700 dark:bg-purple-500/15 dark:text-purple-400",

  admin: "bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-400",

  Artisans: "bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-400",

  supervisor: "bg-orange-50 text-orange-700 dark:bg-orange-500/15 dark:text-orange-400",

  operator: "bg-cyan-50 text-cyan-700 dark:bg-cyan-500/15 dark:text-cyan-400",

  planner: "bg-yellow-50 text-yellow-700 dark:bg-yellow-500/15 dark:text-yellow-400",
...
const getCompanyCode = (user: ApiUser) => {
  if (typeof user.company === "object") {
    return user.company?.companyCode || user.companyCode || user.company_code || "-";
  }
...
const mapApiUserToAdmin = (user: ApiUser): Admin => {
  ...
  return {
    ...
    mobileNumber: user.mobileNumber || user.mobile_number || user.mobile || user.phone || "",
...
export default function AdminManagementTable() {
  ...
  const fetchUsers = async (page = 1) => {
    try {
      ...
      setAdmins(Array.isArray(usersData) ? usersData.map(mapApiUserToAdmin) : []);
    } catch (err) {
                  ...
                  >
                    ...
                    <td className="relative px-4 py-4">
                      <button
                        type="button"
                        onClick={() =>
                          setOpenMenu(openMenu === String(admin.id) ? null : String(admin.id))
                        }
            ...
            {modalType === "edit" && editForm && (
              <div className="space-y-3">
                ...
                <InputBox
                  ...
                  onChange={(value) => setEditForm({ ...editForm, mobileNumber: value })}
                />
                ...
                <div>
                  ...
                  <select
                    ...
                    className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700 outline-none focus:border-blue-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300"
                  >
                ...
                <InputBox
                  ...
                  onChange={(value) => setEditForm({ ...editForm, companyName: value })}
                />
            ...
            {modalType === "view" && (
              <div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <DetailBox label="Role" value={formatRole(selectedAdmin.roleName)} />
                  <DetailBox label="Company Code" value={selectedAdmin.companyCode} />
                  <DetailBox label="Company Name" value={selectedAdmin.companyName} />
                  <DetailBox label="Status" value={selectedAdmin.status} />
                  <DetailBox label="User ID" value={String(selectedAdmin.id)} />
                  <DetailBox label="Created At" value={formatDateTime(selectedAdmin.createdAt)} />
                  <DetailBox label="Updated At" value={formatDateTime(selectedAdmin.updatedAt)} />
                </div>
