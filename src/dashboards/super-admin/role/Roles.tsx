...
import { z } from "zod";
import { Eye, Pencil, Plus, RefreshCw, Search, Trash2, UserCheck, X } from "lucide-react";
...
const normalizeRoleName = (name: string) => name.toLowerCase().replace(/\s+/g, "_").trim();
...
const getUserRole = (user: ApiUser) => {
  const roleValue =
    user.role_name || (typeof user.role === "object" ? user.role?.name : user.role) || "viewer";
...
const mapApiUserToRow = (user: ApiUser, roles: Role[]): UserRow => {
  ...
  const matchedRole = roles.find(
    (role) => String(role.id).trim() === String(user.roleId || user.role_id).trim(),
  );
...
export default function Roles() {
    ...
    try {
      ...
      const mappedRoles: Role[] = data.map((role) => {
        ...
        return {
          ...
          createdAt: formatDate((role as any).createdAt || (role as any).created_at),
          updatedAt: formatDate((role as any).updatedAt || (role as any).updated_at),
        };
      });
  ...
  const handleCreateRole = async () => {
    ...
    try {
      ...
      showSuccessToast(response?.message || "Role created successfully");

      closeAddModal();

      await fetchRoles();
    } catch (error: any) {
      showErrorToast(error?.response?.data?.message || error?.message || "Failed to create role");
    } finally {
  ...
  const handleUpdateRole = async () => {
    ...
    try {
      ...
      showErrorToast(error?.response?.data?.message || error?.message || "Failed to update role");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleDeleteRole = async () => {
    ...
    try {
      ...
      showSuccessToast(response?.message || "Role deleted successfully");

      setDeleteRole(null);

      await fetchRoles();
    } catch (error: any) {
      showErrorToast(error?.response?.data?.message || error?.message || "Failed to delete role");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleAssignRole = async (userId: number | string) => {
    ...
    try {
      ...
      showSuccessToast(response.message || `${selectedRole.name} role assigned successfully`);
    } catch (error: any) {
      showErrorToast(error?.response?.data?.message || error?.message || "Failed to assign role");
    } finally {
      setActionLoading(false);
    }
  };
  return (
    <div className="space-y-6">
      <div className="overflow-hidden rounded-2xl border border-blue-700/30 bg-gradient-to-r from-blue-800 via-blue-700 to-blue-800 px-6 py-6 shadow-lg">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          ...
          <div>
            ...
            <h1 className="text-3xl font-black tracking-tight text-white">Role Management</h1>

            <p className="mt-2 max-w-2xl text-sm font-medium text-blue-100">
              Manage system roles, permissions, and access control settings from one centralized
              location.
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <StatCard title="Inactive Roles" value={String(roles.length - activeRoles)} />
      </div>

      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-[#0f172a]">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Roles List</h2>
            ...
            <tbody>
              {loading ? (
                ...
                filteredRoles.map((role) => (
                  ...
                  >
                    <td className="py-3 font-medium text-gray-900 dark:text-white">{role.name}</td>
                    ...
                    <td className="text-gray-500 dark:text-gray-400">{role.createdAt}</td>

                    <td className="text-gray-500 dark:text-gray-400">{role.updatedAt}</td>
    ...
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="w-full max-w-xl rounded-[32px] border border-slate-200 bg-white p-7 shadow-[0_30px_80px_rgba(0,0,0,0.12)] dark:border-slate-800 dark:bg-[#081028]">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[3px] text-blue-500">ROLE DETAILS</p>

            <h3 className="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{role.name}</h3>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="w-full max-w-md rounded-[32px] bg-white p-7 shadow-[0_30px_80px_rgba(0,0,0,0.12)] dark:bg-[#081028]">
        ...
        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Delete Role</h3>
...
function StatCard({ title, value }: any) {
  return (
    <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#081028]">
      ...
      <h3 className="mt-3 text-4xl font-extrabold text-slate-900 dark:text-white">{value}</h3>
    </div>
  );
}
...
function RoleUsersModal({ role, users, loading, assigning, onClose, onAssign }: any) {
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 p-4 backdrop-blur-md">
      <div className="flex max-h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_30px_80px_rgba(0,0,0,0.12)] dark:border-slate-800 dark:bg-[#081028]">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 p-7 dark:border-slate-800">
          <div>
            <p className="text-xs font-bold uppercase tracking-[3px] text-blue-500">ASSIGN ROLE</p>
