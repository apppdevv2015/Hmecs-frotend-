...
export default function CompanyAdminsPage() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<"All Plans" | PlanType>("All Plans");
  const [selectedCompany, setSelectedCompany] = useState("All Companies");

  const [isStaffModalOpen, setIsStaffModalOpen] = useState(false);
  const [selectedCompanyStaff, setSelectedCompanyStaff] = useState<StaffMember[]>([]);
  ...
  const totalStaff = companies.reduce((total, company) => total + company.staff_count, 0);

  const activeAdmins = companies.filter((company) => company.status === "Active").length;
  ...
  const handleSubmitForm = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const companyPayload: Company = {
      id: formMode === "edit" && editingCompanyId ? editingCompanyId : Date.now(),
      ...
      joined_date:
        formMode === "edit"
          ? companies.find((company) => company.id === editingCompanyId)?.joined_date ||
    ...
    if (formMode === "add") {
      ...
      setCompanies((prev) =>
        prev.map((company) => (company.id === editingCompanyId ? companyPayload : company)),
      );
    }
  ...
  const handleDeleteCompany = () => {
    ...
    setCompanies((prev) => prev.filter((company) => company.id !== deleteCompany.id));
            ...
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-500/10">
                  {formMode === "add" ? <UserPlus size={24} /> : <Pencil size={22} />}
                </div>
            ...
            <form onSubmit={handleSubmitForm} className="grid gap-5 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange("activePlan", e.target.value)}
                  className="w-full rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-900 outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                >
                  <option>Premium</option>
                  <option>Silver</option>
