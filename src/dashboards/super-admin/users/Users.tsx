...
const showToastError = (message: string) => toast.error(message, { position: "top-right" });

const showToastSuccess = (message: string) => toast.success(message, { position: "top-right" });
...
const getCompanyName = (user: ApiUser) => user.company?.name || user.company_name || "—";
...
const mapApiUserToUser = (user: ApiUser): ExtendedUser => {
  ...
  const roleValue =
    typeof user.role === "object" ? user.role?.name : user.role || user.role_name || "operator";
  ...
  return {
    ...
    companyCode: user.company?.companyCode || user.companyCode || user.company_code || "—",
...
function ModalShell({
  ...
  return (
    ...
    >
      <div
        className={`w-full ${maxWidth} rounded-[28px] border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-[#081028] flex flex-col max-h-[90vh]`}
      >
        {/* Header */}
...
function FormField({ label, colSpan, error, as = "input", ...rest }: FormFieldProps) {
  const baseClass =
    "h-[48px] w-full rounded-2xl border bg-white px-4 text-sm outline-none transition focus:border-blue-500 dark:bg-[#0B1739] dark:text-white " +
  ...
  return (
    <div className={colSpan ? "md:col-span-2" : ""}>
      ...
      {as === "select" ? (
        <select className={baseClass} {...(rest as React.SelectHTMLAttributes<HTMLSelectElement>)}>
          {(rest as { children: React.ReactNode }).children}
        </select>
      ) : (
        <input className={baseClass} {...(rest as React.InputHTMLAttributes<HTMLInputElement>)} />
      )}
...
// ========================
// REUSABLE: PhoneField (with selectable country code)
// ========================
// ========================
// REUSABLE: PhoneField (with selectable country code)
// ========================
  ...
  return (
    <>
      ...
      {onConfirm && confirmLabel && (
        <button onClick={onConfirm} disabled={isLoading || disabled} className={confirmClass}>
...
export default function Users() {
  ...
  const [initialFormData, setInitialFormData] = useState<UserFormData>(emptyForm);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return allUsersForFilters.filter((u) => {
      ...
      const matchCompany = companyFilter === ALL_COMPANIES || u.company === companyFilter;
      const matchRole = roleFilter === ALL_ROLES || u.role === roleFilter;
      const matchStatus = statusFilter === ALL_STATUS || u.status === statusFilter.toLowerCase();
  ...
  const startItem = totalFilteredUsers === 0 ? 0 : (currentPage - 1) * USERS_PER_PAGE + 1;
  ...
  const openEditModal = async (user: User) => {
    try {
      setViewLoading(true);
      const response = (await userService.getUserById(user.id)) as ApiUser;
      const firstName = response.firstName || response.first_name || response.fname || "";
      const lastName = response.lastName || response.last_name || response.lname || "";
      const role =
      ...
      showToastError(getErrorMessage(err, "Failed to load user"));
    } finally {
      setViewLoading(false);
    }
  };
    ...
    <div className="min-h-screen p-5">
      <div className="mt-5 rounded-[28px] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#081028]">
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm font-medium text-blue-100">
                Manage platform users, roles, permissions, and account access from one centralized
                location.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="rounded-2xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-semibold text-white backdrop-blur-sm transition-all duration-200 hover:bg-white/20"
              >
                Refresh
              </button>
              <button
        ...
        <Pagination
          ...
          onNext={() => setCurrentPage((p) => Math.min(p + 1, filteredTotalPages))}
        />
      </div>
...
function UsersFilters({
  ...
  const selectClass =
    "h-[48px] rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-700 outline-none transition focus:border-blue-500 dark:border-slate-700 dark:bg-[#0B1739] dark:text-white";
...
function UsersTable({
  ...
  const thClass = "px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500";
    ...
    <div className="w-full overflow-x-auto">
      <table className="w-full min-w-[1100px] text-left">
        <thead className="border-b border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-[#0B1739]/30">
          <tr>
            {["User", "Email", "Phone", "Role", "Company", "Status", "Last Login"].map((h) => (
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {users.map((user) => (
            <tr key={user.id} className="transition hover:bg-slate-50 dark:hover:bg-[#0B1739]/40">
              ...
              <td className="px-6 py-5 text-sm text-slate-600 dark:text-slate-300">{user.phone}</td>
              <td className="px-6 py-5">
                <span
...
function UserModal({
  ...
  const [flowType, setFlowType] = useState<UserFlowType>(initialFormData.flowType ?? "existing");
  const [loading, setLoading] = useState(false);

  const isNewCompanyAdmin = flowType === "new_company_admin";

  const activeSchema = isNewCompanyAdmin ? newCompanyAdminFormSchema : existingUserFormSchema;
    ...
    try {
      ...
      if (mode === "add") {
        const payload: AddUserPayload = {
          ...
          role_name: flowType === "existing" ? getApiRoleName(form.role ?? "Operator") : "admin",
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  >
                    {!isNewCompanyAdmin && <span className="h-2 w-2 rounded-full bg-blue-500" />}
                  </span>
                  <span
                ...
                <p className="ml-6 text-xs text-slate-500 dark:text-slate-400">
                  Assign user to an already registered company. Select company & role below.
                </p>
              </button>
              ...
              >
                ...
                <p className="ml-6 text-xs text-slate-500 dark:text-slate-400">
                  Create an Admin who will register their company & purchase a plan independently.
                </p>
              </button>
            </div>
          </div>
          ...
          >
            ...
            {companies.map((company) => (
              <option key={company.id} value={company.id}>
                {company.name || company.company_name || company.companyName || "Unnamed Company"}
              </option>
            ))}
          </FormField>
        )}
        ...
        {isNewCompanyAdmin && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-500/20 dark:bg-amber-500/10 md:col-span-2">
            ...
            <p className="mt-0.5 text-xs text-amber-600 dark:text-amber-300">
              This Admin account will be created without a company. They can register their company
              and purchase a plan after logging in.
            </p>
          </div>
        )}
...
function SuperAdminModal({
  ...
  const onSubmit = async (form: SuperAdminFormData) => {
    try {
      ...
      await userService.addUser(payload);
      showToastSuccess("Super Admin created successfully");
      onClose();
      onCreated();
    } catch (err) {
      showToastError(getErrorMessage(err, "Failed to create Sub Super Admin"));
    } finally {
      setLoading(false);
    }
  };
...
function ViewUserModal({ user, onClose }: { user: ExtendedUser; onClose: () => void }) {
  return (
    <ModalShell
      title="User Details"
      subtitle="View complete user information"
      onClose={onClose}
      footer={<ModalFooterButtons onCancel={onClose} cancelLabel="Close" confirmLabel="" />}
    >
