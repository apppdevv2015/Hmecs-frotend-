...
export default function OperatorsMechanicsChart() {
  ...
  useEffect(() => {
    ...
    const fetchMetrics = async () => {
      try {
        setLoading(true);
        const res = await superAdminMachineService.getDashboardMetrics();
        if (isMounted) {
          setData([
            { name: "Operators", value: res?.totalOperators || 0 },
            { name: "Mechanics", value: res?.totalMechanics || 0 },
          ]);
        }
      } catch (err) {
        console.error("Error loading workforce metrics:", err);
      } finally {
    ...
    <section className="flex h-full min-h-[360px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-4 flex flex-1 flex-col justify-between">
        {loading ? (
          ...
          <div className="flex flex-1 items-center justify-center py-10 text-xs font-semibold text-slate-500 dark:text-slate-400">
            No workforce members registered.
          </div>
        ) : (
          <>
            <div className="relative mx-auto h-[190px] w-full outline-none [&_.recharts-surface]:outline-none [&_.recharts-wrapper]:outline-none">
              ...
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-lg font-bold text-slate-950 dark:text-white">{total}</p>
                </div>
              </div>
            </div>
