...
import { Activity, AlertTriangle, Filter, Search, Settings, Truck, Wrench } from "lucide-react";
...
const statusStyles: Record<ComponentStatus, string> = {
  Healthy: "bg-green-100 text-green-700 dark:bg-green-500/15 dark:text-green-300",
...
export default function SuperAdminComponents() {
  ...
  const filteredComponents = useMemo(() => {
    return componentsData.filter((item) => {
      const searchableText =
        `${item.componentName} ${item.machineName} ${item.machineCode} ${item.companyName} ${item.location} ${item.category}`.toLowerCase();
  ...
  const paginatedComponents = filteredComponents.slice((page - 1) * pageSize, page * pageSize);

  const totalComponents = componentsData.length;
  const healthyCount = componentsData.filter((item) => item.status === "Healthy").length;
  const warningCount = componentsData.filter((item) => item.status === "Warning").length;
  const criticalCount = componentsData.filter((item) => item.status === "Critical").length;

  const averageHealth = Math.round(
    componentsData.reduce((total, item) => total + item.health, 0) / componentsData.length,
  );
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        ...
        <p className="mt-2 max-w-3xl text-sm text-gray-500 dark:text-gray-400">
          Monitor all company machines, component health, service dates, and critical component
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Average Health</p>
              <h2 className="mt-2 text-2xl font-bold text-gray-900 dark:text-white">
                {averageHealth}%
              </h2>
            </div>
      ...
      <div className="mt-6 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="flex flex-col gap-4 border-b border-gray-200 p-4 dark:border-gray-800 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Components List</h2>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-2 w-28 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
                          <div
                            className={`h-full rounded-full ${healthBarStyles(item.health)}`}
