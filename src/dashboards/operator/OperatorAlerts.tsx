...
const fetchAssignedMachine = async (
  _operatorId: string,
): Promise<ApiResponse<AssignedMachine>> => {
...
const getSeverityLabel = (c: ComponentCondition) =>
  ({ 5: "Critical", 4: "Warning", 3: "Moderate", 2: "Good", 1: "Excellent" })[
    c
  ];
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-5 py-4 animate-pulse flex flex-col gap-3">
      ...
      <div className="flex gap-4 pt-2 border-t border-slate-100 dark:border-slate-800">
        {[...Array(3)].map((_, i) => (
          <div
            key={i}
            className="h-2.5 w-24 rounded bg-slate-100 dark:bg-slate-800"
          />
        ))}
      </div>
    </div>
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-2.5">
          <div
            className={`mt-0.5 rounded-lg p-2 shrink-0 ${s.iconBg} ${s.iconColor}`}
          >
        ...
        <span
          className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-medium ${s.badge}`}
        >
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2 px-0.5">
        ...
        <p
          className={`text-[11px] font-semibold uppercase tracking-widest ${s.heading}`}
        >
...
const SummaryChip = ({
  ...
  <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-3 flex flex-col gap-2">
    <div className={`w-fit rounded-md p-1.5 ${iconBg} ${iconColor}`}>
      {icon}
    </div>
