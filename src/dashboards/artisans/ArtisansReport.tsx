...
const reportSchema = z.object({
  selectedMachineId: z.string().trim().min(1, "Please select the related machine."),
  ...
  sendTo: z.array(z.enum(["Supervisor", "Admin"])).min(1, "Select at least one person."),
...
const ArtisansTasks = () => {
  ...
  const [selectedReport, setSelectedReport] = useState<ArtisansReport | null>(null);
    ...
    <div className="space-y-6 bg-slate-50 dark:bg-slate-950 p-4 sm:p-6">
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100">
              Submit maintenance and inspection reports for review and record keeping.
            </p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr] xl:grid-cols-[1.1fr_1fr]">
        ...
        >
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Provide accurate details so supervisors and admin can review quickly.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  updateField("selectedMachineId", e.target.value);
                }}
                className={`w-full rounded-2xl border bg-white dark:bg-slate-800 px-4 py-3 text-sm text-slate-800 dark:text-slate-100 outline-none transition focus:ring-4 ${
                  formErrors.selectedMachineId
                    ? "border-red-300 dark:border-red-500/50 focus:border-red-400 focus:ring-red-500/20"
                    : "border-slate-200 dark:border-slate-700 focus:border-blue-400 dark:focus:border-blue-500 focus:ring-blue-500/10"
                }`}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full rounded-2xl border bg-white dark:bg-slate-800 px-4 py-3 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 outline-none transition focus:ring-4 ${
              ...
              <div className="mt-1 min-h-[20px]">
                <p className="text-xs font-medium text-red-500">{formErrors.issueTitle || ""}</p>
              </div>
            </div>
            {/* Priority + Report To */}
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-3 text-sm text-slate-800 dark:text-slate-100 outline-none transition focus:border-blue-400 dark:focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
                  style={{ colorScheme: "light dark" }}
                >
                  <option>Low</option>
              ...
              <div>
                ...
                {formErrors.sendTo && (
                  <p className="mt-1 text-xs font-medium text-red-500">{formErrors.sendTo}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="mt-1 flex items-center justify-between">
                {formErrors.description ? (
                  <p className="text-xs font-medium text-red-500">{formErrors.description}</p>
                ) : (
                  <span />
                )}
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">
                Action Taken / Recommendation <span className="text-red-500">*</span>
              </label>
              <textarea
                value={actionTaken}
                onChange={(e) => {
              ...
              <div className="mt-1 flex items-center justify-between">
                {formErrors.actionTaken ? (
                  <p className="text-xs font-medium text-red-500">{formErrors.actionTaken}</p>
                ) : (
                  <span />
                )}
                <p className="text-xs text-slate-400 dark:text-slate-500">
          ...
          {formErrors.media && (
            <p className="mt-2 text-xs font-medium text-red-500">{formErrors.media}</p>
          )}
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 p-4">
              ...
              <div className="space-y-2">
                {images.map((file) => (
                  ...
                  >
                    <span className="flex items-center gap-2 truncate text-slate-600 dark:text-slate-300">
                      <ImageIcon size={16} className="shrink-0 text-blue-500 dark:text-blue-400" />
                      <span className="truncate">{file.name}</span>
                      <span className="shrink-0 text-xs text-slate-400">
                        ({(file.size / 1024 / 1024).toFixed(1)} MB)
                      </span>
                    </span>
                    <button type="button" onClick={() => removeFile(file.name, "image")}>
                      <X
                        size={16}
                ...
                {videos.map((file) => (
                  ...
                  >
                    <span className="flex items-center gap-2 truncate text-slate-600 dark:text-slate-300">
                      <FileVideo size={16} className="shrink-0 text-blue-500 dark:text-blue-400" />
                      <span className="truncate">{file.name}</span>
                      <span className="shrink-0 text-xs text-slate-400">
                        ({(file.size / 1024 / 1024).toFixed(1)} MB)
                      </span>
                    </span>
                    <button type="button" onClick={() => removeFile(file.name, "video")}>
                      <X
                        size={16}
                        className="text-slate-400 hover:text-red-500 transition-colors"
                      />
          ...
          {formErrors.submit && (
            <p className="mt-3 text-sm font-medium text-red-500">{formErrors.submit}</p>
          )}
        ...
        <div className="flex min-w-0 flex-col rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          <div className="border-b border-slate-100 dark:border-slate-800 p-5">
            <h2 className="text-lg font-bold text-slate-950 dark:text-white">Report History</h2>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 py-2.5 pl-9 pr-3 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 outline-none transition focus:border-blue-400 dark:focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
                />
              </div>
              ...
              <select
                ...
                className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm text-slate-800 dark:text-slate-100 outline-none transition focus:border-blue-400 dark:focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
                style={{ colorScheme: "light dark" }}
              >
                    ...
                    {paginatedReports.map((report) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          <p className="text-xs text-slate-400">{report.vehicleNo}</p>
                        </td>
                        <td className="px-4 py-3">
                          <span
                        ...
                        <td className="px-4 py-3 text-right">
                          <Eye size={16} className="inline text-blue-600 dark:text-blue-400" />
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        >
          ...
          >
            ...
            <div className="mt-5 space-y-4 text-sm">
              <div>
                <p className="font-semibold text-slate-700 dark:text-slate-300">Machine</p>
              ...
              <div>
                <p className="font-semibold text-slate-700 dark:text-slate-300">Reported To</p>
                <p className="mt-1 text-slate-600 dark:text-slate-400">
                  {selectedReport.sendTo.join(", ")}
                </p>
              </div>
              <div>
                <p className="font-semibold text-slate-700 dark:text-slate-300">Submitted On</p>
                <p className="mt-1 text-slate-600 dark:text-slate-400">
              ...
              {(selectedReport.images.length > 0 || selectedReport.videos.length > 0) && (
                <div>
                  <p className="font-semibold text-slate-700 dark:text-slate-300">Attachments</p>
