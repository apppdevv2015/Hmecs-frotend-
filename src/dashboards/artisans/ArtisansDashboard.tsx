...
========================== */
  ...
  useEffect(() => {
    ...
    const drilldownData = [
      {
        dataGroupId: "healthy",
        data: machineHealthState.filter((m) => m.health >= 80).map((m) => [m.machine, m.health]),
      },
...

      {
        ...
        data: machineHealthState.filter((m) => m.health < 60).map((m) => [m.machine, m.health]),
      },
    ];
    ...
    (window as any).chartClickHandler = (params: any, chart: any) => {
      ...
      const subData = drilldownData.find((d) => d.dataGroupId === params.data.groupId);
            ...
            <div className="relative z-10 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100">
                  Monitor live maintenance tasks, machine health predictions, component risk
                  analysis, maintenance schedules and operational response activities from a
                  centralized Artisans Operations dashboard.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {alertsState.slice(0, 3).map((alert) => (
                ...
                >
                  ...
                  <p className="shrink-0 text-xs font-bold text-slate-400">{alert.time}</p>
                </button>
              ))}
            </div>
          </Panel>
              ...
              {maintenance.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-bold uppercase tracking-[0.16em]">{item.title}</p>
                      <h3 className="mt-3 text-3xl font-extrabold tracking-tight">{item.value}</h3>
                    </div>
                    {item.icon}
                  </div>
                </button>
              ...
              {machineHealthState.map((item) => (
                ...
                >
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <p className={`text-sm font-extrabold ${healthTextColor(item.health)}`}>
                      {item.health}%
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setTaskStatusFilter(event.target.value as TaskStatus | "All")
                        }
                          ...
                          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                {task.id} • {task.location} • Due: {task.dueDate}
                              </p>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              >
                                {task.status === "Pending" && <PlayCircle size={15} />}
                                {task.status === "In Progress" && <CheckCircle size={15} />}
                    ...
                    {machineHealthState.map((machine) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <span
                            className={`text-xl font-extrabold ${healthTextColor(machine.health)}`}
                          >
                            {machine.health}%
                          </span>
                        </div>
                        ...
                        <div className="mt-4 h-2.5 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
                          <div
                            className={`h-full rounded-full ${healthColor(machine.health)}`}
                            style={{ width: `${machine.health}%` }}
                          />
                        </div>

                        <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                          <SmallMetric label="Runtime" value={machine.runtime} />
                          <SmallMetric label="Engine" value={machine.engineTemp} />
                          <SmallMetric label="Hydraulic" value={machine.hydraulicPressure} />
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            <div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-800 dark:bg-[#0b1728]">
              ...
              <div className="mt-5 space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <InfoCard label="Assigned Date" value={selectedTask.assignedDate} />
                  <InfoCard label="Due Date" value={selectedTask.dueDate} />
                  <InfoCard label="Current Status" value={selectedTask.status} />
                </div>
              ...
              <div className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold">{selectedAlert.issue}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-800 dark:bg-[#0b1728]">
              ...
              <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-white/[0.03]">
                ...
                <div className="mt-5 h-3 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
                  <div
                    className={`h-full rounded-full ${healthColor(selectedMachine.health)}`}
          ...
          <div className="fixed inset-0 z-[1000000] flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-800 dark:bg-[#0b1728]">
              ...
              <h2 className="mt-4 text-xl font-extrabold text-slate-950 dark:text-white">
                {confirmTask.status === "Pending" ? "Start this task?" : "Complete this task?"}
              </h2>
...
function SmallMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-[#101f33]">
      ...
      <p className="mt-1 text-xs font-extrabold text-slate-950 dark:text-white">{value}</p>
    </div>
  );
}
