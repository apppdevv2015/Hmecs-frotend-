...
type AlertComponent =
  "Engine" | "Hydraulic" | "Suspension" | "Tyre" | "Battery" | "Brake" | "Coolant" | "Transmission";
...
const SEVERITY_STYLES: Record<AlertSeverity, { badge: string; bar: string; dot: string }> = {
  Critical: {
    badge:
...
export default function SupervisorAlerts() {
  ...
  const [componentFilter, setComponentFilter] = useState<AlertComponent | "All">("All");
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return alerts.filter((alert) => {
      ...
      const matchesComponent = componentFilter === "All" || alert.component === componentFilter;

      return matchesSearch && matchesSeverity && matchesStatus && matchesComponent;
    });
  ...
  const paginatedAlerts = filteredAlerts.slice(startIndex, startIndex + PAGE_SIZE);
    ...
    <div className="min-h-full space-y-6 bg-slate-50 p-4 dark:bg-slate-950 md:p-6">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Track component-level faults across your fleet and manage resolution status in real time.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Critical</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-1 text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.open}</p>
        </div>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-lg border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-blue-950"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              onChange={(event) => {
                setComponentFilter(event.target.value as AlertComponent | "All");
                setCurrentPage(1);
              }}
              className="h-10 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:focus:ring-blue-950"
            >
            ...
            <select
              value={statusFilter}
              onChange={(event) => {
                setStatusFilter(event.target.value as AlertStatus | "All");
                setCurrentPage(1);
              }}
              className="h-10 rounded-lg border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:focus:ring-blue-950"
            >
      ...
      <div className="space-y-3">
        {paginatedAlerts.map((alert) => {
          ...
          return (
            ...
            >
              <span className={`absolute inset-y-0 left-0 w-1 ${severityStyle.bar}`} />
        ...
        {paginatedAlerts.length === 0 && (
          <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center dark:border-slate-700 dark:bg-slate-900">
            ...
            <p className="mt-1 text-xs text-slate-400">Try adjusting search or clearing filters.</p>
          </div>
        )}
      </div>
      {/* Pagination */}
