...
// (Replace with actual auth/session data when backend is ready)
// BACKEND TODO: get from auth context / session
// ════════════════════════════════════════════════════════════════════════════
...
// e.g. const res = await fetch("/api/reports"); const data = await res.json();
// BACKEND TODO: scope these endpoints to reports submitted under this
// supervisor's fleet/team only (e.g. /api/supervisor/:id/reports)
// ════════════════════════════════════════════════════════════════════════════
...
const canAct = (status: Report["status"]) => status === "pending" || status === "reviewed";
        ...
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1 leading-relaxed">{config.message}</p>
            </div>
          </div>
        </div>
...
function ReportModal({
  ...
  report: Report;
  onClose: () => void;
  onAction: (id: string, status: Report["status"], note?: string) => Promise<void>;
}) {
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-blue-100">
          ...
          <div className="flex items-start justify-between p-6 border-b border-blue-50">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-800 leading-tight">{report.title}</h2>
          ...
          <div className="grid grid-cols-2 gap-4 px-6 pt-4">
            <div className="bg-blue-50 rounded-xl p-3">
              ...
              <p className="text-sm font-semibold text-slate-700 mt-1">{report.submittedBy}</p>
            </div>
            <div className="bg-blue-50 rounded-xl p-3">
              <p className="text-xs text-blue-400 font-medium uppercase tracking-wide">
                Report Type
              </p>
              <p className="text-sm font-semibold text-slate-700 mt-1 capitalize">{report.type}</p>
            </div>
          </div>
          ...
          {/* Tags */}
          <div className="px-6 pt-4">
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide mb-2">Tags</p>
          ...
          {canAct(report.status) && (
            <div className="px-6 pt-4">
              <p className="text-xs text-slate-400 font-medium uppercase tracking-wide mb-2">
                Remark / Note <span className="normal-case text-slate-300">(optional)</span>
              </p>
              <textarea
                value={note}
      ...
      {confirm && (
        <ConfirmDialog config={confirm} onCancel={() => setConfirm(null)} loading={submitting} />
      )}
    </>
  );
}
  ...
  return (
    ...
    >
      <td className="px-4 py-3.5">
        <div className="flex items-center gap-3">
          <span className="text-xl flex-shrink-0">{TYPE_ICONS[report.type]}</span>
          <div>
      ...
      <td className="px-4 py-3.5">
        <p className="text-sm text-slate-700 font-medium">{report.submittedBy}</p>
        <RoleBadge role={report.role} />
      </td>
      <td className="px-4 py-3.5">
        <PriorityBadge priority={report.priority} />
      ...
      <td className="px-4 py-3.5 text-sm text-slate-400">{report.date}</td>
      <td className="px-4 py-3.5">
...
function HistoryCard({
  ...
  return (
    <div className="flex gap-4 cursor-pointer group" onClick={() => onClick(entry)}>
      <div className="flex flex-col items-center">
        <div
        ...
        {!isLast && <div className="w-0.5 flex-1 bg-blue-100 mt-1" style={{ minHeight: 24 }} />}
      </div>
      <div
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div>
            <p className="text-sm font-bold text-slate-800">{entry.reportTitle}</p>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-600 font-medium">{entry.performedBy}</span>
          </div>
...
function ReportsPage({
  ...
  const hasFilters =
    filterStatus !== "all" || filterType !== "all" || filterRole !== "all" || !!search;
  const clearFilters = () => {
    setFS("all");
    setFT("all");
    setFR("all");
  ...
  return (
    <div>
      ...
      <div className="relative mb-6 overflow-hidden rounded-[28px] border border-indigo-300/20 bg-gradient-to-r from-[#3B37E6] via-[#3730D9] to-[#2E2AD9] px-6 py-6 shadow-sm">
        ...
        <div className="relative">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-black tracking-tight text-white">Report Approvals</h1>

              <p className="mt-2 text-sm text-blue-100">
                Reports submitted by your Artisans & Operators, awaiting your review
              </p>
            </div>
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
        ...
        {/* Filters */}
        <div className="flex flex-wrap items-center gap-3 px-4 py-3 bg-slate-50/70 border-b border-blue-50">
          ...
          {hasFilters && (
            <button
              ...
              className="px-3 py-2 text-xs font-medium text-blue-600 hover:bg-blue-50 rounded-lg border border-blue-100 transition-colors"
            >
              Clear filters
            </button>
          )}
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="bg-blue-50/50">
                {["Report", "Submitted By", "Priority", "Status", "Date", ""].map((h, i) => (
                  <th
                    key={i}
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden">
        <div className="flex items-start justify-between p-6 border-b border-slate-100">
          <div>
            <h2 className="text-lg font-bold text-slate-800">{entry.reportTitle}</h2>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-slate-50 rounded-xl p-4">
              ...
              <p className="text-sm font-semibold text-slate-700 mt-2">{entry.performedBy}</p>
              <div className="mt-2">
                <RoleBadge role={entry.performedByRole} />
              </div>
            </div>
          ...
          {entry.note && (
            <div>
              ...
              <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">{entry.note}</div>
            </div>
          )}
        </div>
      </div>
...
function HistoryPage({
  ...
  const hasFilters =
    filterAction !== "all" || filterRole !== "all" || filterDate !== "all" || !!search;
  const clearFilters = () => {
    setFilterAction("all");
    setFilterRole("all");
    setFilterDate("all");
  ...
  filtered.forEach((h) => {
    const key = groupBy === "date" ? h.date : `${h.reportId}|||${h.reportTitle}`;
    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Complete audit trail of every review, approval and rejection you've made
            </p>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm mb-6">
        ...
        <div className="px-4 py-2 bg-slate-50/50 border-b border-blue-50">
          <p className="text-xs text-slate-400">
            <span className="font-semibold text-slate-600">{filtered.length}</span> entries found
          </p>
        </div>
        ...
        <div className="px-6 py-5">
          {loading ? (
            Array.from({ length: 4 }).map((_, i) => <SkeletonHistoryCard key={i} />)
          ) : filtered.length === 0 ? (
            <EmptyState
              icon={hasFilters ? "🔎" : "🗂️"}
            ...
            groupKeys.map((gKey) => {
              ...
              const displayLabel = groupBy === "date" ? gKey : gKey.split("|||")[1];
              return (
  ...
  const handleAction = useCallback(
    async (id: string, status: Report["status"], note?: string) => {
      try {
        ...
        push(types[status], labels[status], `"${updated.title}" has been ${status} successfully.`);
      } catch {
        push("error", "Action Failed", "Something went wrong. Please try again.");
      }
    },
    [push],
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-6">
        {activePage === "reports" ? (
          <ReportsPage reports={reports} loading={loadingReports} onAction={handleAction} />
        ) : (
          <HistoryPage history={history} reports={reports} loading={loadingHistory} />
        )}
      </div>
