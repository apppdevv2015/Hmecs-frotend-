...
const getConditionLabel = (condition: number) => {
  switch (condition) {
    ...
    case 3:
      return {
        text: "Monitor",
        class: "border-yellow-200 bg-yellow-50 text-yellow-700 dark:border-yellow-500/20 dark:bg-yellow-500/10 dark:text-yellow-300",
      };
    case 4:
      return {
        text: "Warning",
        class: "border-orange-200 bg-orange-50 text-orange-700 dark:border-orange-500/20 dark:bg-orange-500/10 dark:text-orange-300",
      };
    case 5:
      return {
        text: "Critical",
        class: "border-red-200 bg-red-50 text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-300",
      };
    default:
      return {
        text: "Unknown",
        class: "border-slate-200 bg-slate-50 text-slate-700 dark:border-slate-800 dark:bg-slate-900/10 dark:text-slate-300",
      };
  }
};
                  ...
                  <tbody className="divide-y divide-blue-50/50 dark:divide-slate-800/50">
                    {filteredComponents.map((component) => {
                      ...
                      const remainingHours = Math.max(
                        0,
                        component.plannedLife - Math.max(0, component.currentHours - component.installHours)
                      );
                      
                      const conditionInfo = getConditionLabel(component.condition);
                      
                      const riskStatus = component.intelligence?.riskStatus || (
                        component.condition >= 5 || lifeUsed >= 95 ? "Critical" :
                        component.condition >= 4 || lifeUsed >= 85 ? "Warning" :
                        component.condition >= 3 || lifeUsed >= 70 ? "Monitor" : "Healthy"
                      );
                      
                      const riskColorClass = 
                        riskStatus === "Critical" ? "border-red-200 bg-red-50 text-red-700 dark:border-red-500/20 dark:bg-red-500/10 dark:text-red-300" :
                        riskStatus === "Warning" ? "border-orange-200 bg-orange-50 text-orange-700 dark:border-orange-500/20 dark:bg-orange-500/10 dark:text-orange-300" :
                        riskStatus === "Monitor" ? "border-yellow-200 bg-yellow-50 text-yellow-700 dark:border-yellow-500/20 dark:bg-yellow-500/10 dark:text-yellow-300" :
                        "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300";

                      return (
                        <tr key={component.id} className="transition hover:bg-slate-50/50 dark:hover:bg-slate-900/40">
                          ...
                          <td className="px-6 py-4 text-sm font-black text-slate-800 dark:text-slate-200">
                            {Math.max(0, component.currentHours - component.installHours).toLocaleString()} h
                          </td>
                          <td className="px-6 py-4">
                              ...
                              <div className="h-2 overflow-hidden rounded-full bg-blue-50 dark:bg-slate-800">
                                <div
                                  className={`h-full rounded-full transition-all duration-500 ${
                                    lifeUsed >= 90 ? "bg-red-500" :
                                    lifeUsed >= 75 ? "bg-orange-500" :
                                    "bg-emerald-500"
                                  }`}
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${conditionInfo.class}`}>
                              {conditionInfo.text}
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-wide ${riskColorClass}`}>
                              {riskStatus}
                            </span>
                          </td>
      ...
      <div className="rounded-2xl border border-blue-100 bg-[#f8fbff] p-4 dark:border-slate-800 dark:bg-slate-900">
        <div className="mb-2 flex items-end justify-between">
          <div>
            ...
            <h4 className="text-3xl font-black text-slate-950 dark:text-white">
              {health}%
            </h4>
          </div>
        ...
        <div className="px-6 py-6">
          ...
          <div className="mb-5 rounded-3xl border border-blue-100 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
            ...
            <div className="mt-2 flex items-end justify-between">
              <h3 className="text-5xl font-black text-slate-950 dark:text-white">
                {health}%
              </h3>
