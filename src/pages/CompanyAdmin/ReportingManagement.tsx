...
// TYPES
// ════════════════════════════════════════════════════════════════════════════
type Role = "supervisor" | "operator" | "engineer";
...
const DB: { reports: Report[]; history: HistoryEntry[] } = {
  reports: [
    ...
    },
    {
      ...
      shift: "evening",
      description:
        "Pressure relief valve V-12 triggered at 14:32. System pressure reached 8.4 bar (limit: 8.0 bar). Immediate shutdown initiated. Root cause under investigation by engineering team.",
...
const ROLE_CFG: Record<Role, { label: string; color: string; bg: string }> = {
  ...
  engineer: { label: "Engineer", color: "text-sky-700", bg: "bg-sky-100" },
};
...
const roleCanApprove = (r: Role) => r === "supervisor";
const roleCanReview = (r: Role) => r === "supervisor" || r === "engineer";
...
// ════════════════════════════════════════════════════════════════════════════
// REUSABLE — SPINNER BUTTON (for loading states inside modals)
// ════════════════════════════════════════════════════════════════════════════
...
function ReportModal({
  ...
  const handleAction = async (status: Report["status"]) => {
    if (status === "rejected") {
      setConfirm({
        title: "Reject this report?",
        message: `You are about to reject "${report.title}". This action will be logged in the activity history and the submitter will be notified.`,
...
function ReportsPage({
  ...
  const filtered = reports.filter((r) => {
    ...
    if (activeTab === "mine" && r.role !== activeRole) return false;
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <EmptyState
                      ...
                      message={
                        hasFilters
                          ...
                          : "Reports submitted by operators and engineers will appear here."
                      }
                      action={
                        hasFilters
                          ? {
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 px-2">
        ...
        <div className="ml-auto text-xs text-slate-400">
          <span className={`font-semibold ${ROLE_CFG[activeRole].color}`}>
            {ROLE_CFG[activeRole].label}
          </span>
          {" · "}
          {roleCanApprove(activeRole)
            ? "Can approve & reject"
            : roleCanReview(activeRole)
              ? "Can review only"
              : "View only"}
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Complete audit trail of all report actions and status changes
            </p>
          </div>
            ...
            <EmptyState
              ...
              message={
                hasFilters
                  ...
                  : "When reports are reviewed, approved, or rejected, the activity will appear here."
              }
              action={
                hasFilters
...
export default function ReportDashboard() {
  ...
  const ROLES: Role[] = ["supervisor", "operator", "engineer"];
    ...
    async (id: string, status: Report["status"], note?: string) => {
      try {
        const { report: updated, historyEntry } = await api.updateReportStatus(
          id,
          status,
          { name: "Admin User", role: activeRole },
          note,
        );
