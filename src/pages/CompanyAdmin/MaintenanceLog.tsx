...
export default function MaintenanceLog({
  ...
  useEffect(() => {
    async function loadDynamicDropdowns() {
      try {
        ...
        const userRes = await userService.getUsers({ limit: 1000 });
        let usersList: any[] = [];
        ...
        const filteredEngineers = usersList
          .filter((u: any) => {
            const roleName = (typeof u.role === "object" ? u.role?.name : u.role_name || u.role) || "";
        ...
        const machineRes = await machineService.getMachines();
        const machinesList = Array.isArray(machineRes) ? machineRes : (machineRes?.data || []);
        setDbMachines(machinesList);

        const componentRes = await componentService.getComponents();
        const componentsList = Array.isArray(componentRes) ? componentRes : (componentRes?.data || []);
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((item) => {
      ...
      const matchesTechnician =
        ...
        item.technician === selectedTechnician;

      const matchesStatus =
        selectedStatus === "All Status" || item.status === selectedStatus;

      const matchesMachine =
        selectedMachine === "All Machines" || item.machine === selectedMachine;
  ...
  const totalDowntime = useMemo(() => {
    return filteredEvents.reduce(
      (sum, item) => sum + getNumberFromText(item.downtime),
      0
    );
  }, [filteredEvents]);
  ...
  const handleCreateEntry = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const selectedMachineObj = dbMachines.find((m) => m.name === form.machine);
      const selectedComponentObj = dbComponents.find((c) => (c.description || c.category) === form.component);
      ...
      await maintenanceService.createLog({
        ...
        downtime: form.status === "Open" ? (form.dueDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]) : (form.downtime || "0 hrs"),
        status: form.status
      });
  ...
  const handleUpdateEntry = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const selectedComponentObj = dbComponents.find((c) => (c.description || c.category) === editForm.component);
      ...
      await maintenanceService.updateLog(String(editingEvent?.id), {
        ...
        downtime: editForm.status === "Open" ? (editForm.dueDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]) : (editForm.downtime || "0 hrs"),
      ...
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl dark:bg-slate-800">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs font-bold text-slate-400">
              Add a new maintenance log record.
            </p>
          </div>
        ...
        >
          ...
          <label className="flex flex-col">
            ...
            <select
              value={form.machine}
              onChange={(e) => setForm((prev) => ({ ...prev, machine: e.target.value, component: "" }))}
          ...
          <label className="flex flex-col">
            ...
            >
              ...
              {engineers.length === 0 && (
                <option disabled>No engineers found. Please register staff with role 'Engineer' first.</option>
              )}
            </select>
          </label>
          ...
          {form.status === "Open" ? (
            <label className="flex flex-col">
              ...
              >
                <Calendar size={16} className="absolute left-4 text-slate-400 pointer-events-none" />
                <input
                  type="date"
                  value={form.dueDate}
        ...
        >
          <label className="flex flex-col">
            ...
            <select
              ...
              onChange={(e) => setEditForm((prev) => ({ ...prev, machine: e.target.value, component: "" }))}
              className="w-full rounded-2xl border border-slate-100 bg-white px-4 py-3.5 text-xs font-bold text-slate-900 outline-none focus:ring-2 focus:ring-purple-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
            >
              <option value="">Select Machine</option>
              {dbMachines.map((m) => (
          ...
          {editForm.status === "Open" ? (
            <label className="flex flex-col">
              ...
              >
                ...
                <input
                  type="date"
                  value={editForm.dueDate || (editForm.downtime && editForm.downtime.includes("-") ? editForm.downtime : "")}
                      ...
                      <td className="px-10 py-7 text-xs font-bold text-slate-500">
                        {item.status === "Open" && item.downtime && item.downtime.includes("-") ? (
                          <span className="inline-flex items-center gap-1.5 text-orange-600 dark:text-orange-400 font-extrabold">
                            Due: {new Date(item.downtime).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                          </span>
                        ) : (
                          item.downtime
                        )}
