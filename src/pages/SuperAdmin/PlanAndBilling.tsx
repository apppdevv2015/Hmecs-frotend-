          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {invoice.items.map((item, index) => (
                ...
                >
                  <td className="px-4 py-5 align-top text-sm font-black">
                    {item.item}
                  </td>
        ...
        <div className="mt-7 grid gap-8 md:grid-cols-2">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-600">
              Payment is linked with subscription activation. Invoice record is
              saved automatically in finance history.
            </p>

            <div className="mt-5 rounded-2xl bg-orange-50 p-4 text-sm text-orange-700">
              <p className="font-black">Note</p>
              <p className="mt-1">
                This invoice is generated for HME subscription and billing
                record.
              </p>
            </div>
          </div>
          ...
          <div className="ml-auto w-full max-w-sm space-y-3 text-sm">
            ...
            <AmountRow
              label="Paid to Date"
              value={formatCurrency(invoice.paidToDate)}
            />
            <AmountRow
              label="Partial Due"
              value={formatCurrency(partialDue)}
              highlight
            />
...
const AmountRow = ({ label, value, strong, highlight }: AmountRowProps) => {
  return (
    ...
    >
      <span
        className={`${strong || highlight ? "font-black" : "font-semibold"}`}
      >
        {label}
      </span>
