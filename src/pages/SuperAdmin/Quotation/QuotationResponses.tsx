...
type QuotationResponseStatus = "SENT" | "ACCEPTED" | "REJECTED" | "EXPIRED";
...
const RESPONSE_STATUSES: readonly QuotationResponseStatus[] = [
  "SENT",
  "ACCEPTED",
  "REJECTED",
  "EXPIRED",
...
function isResponseStatus(status: string): status is QuotationResponseStatus {
  return (RESPONSE_STATUSES as readonly string[]).includes(status);
}

function mapQuotationToResponse(
  quotation: Quotation,
): QuotationResponse | null {
  // DRAFT quotation abhi company ko bheji hi nahi gayi, is list mein show nahi honi chahiye
  if (!isResponseStatus(quotation.status)) {
    return null;
  }
  ...
  return {
    ...
    sentAt: quotation.sentAt ?? quotation.createdAt,
    respondedAt: quotation.acceptedAt ?? quotation.rejectedAt ?? "",
    status: quotation.status,
    rejectionReason: quotation.notes ?? "",
    quotationType: quotation.tier,
    contractDuration: quotation.contractDuration,
    quotationAmount: Number(quotation.totalAmount ?? 0),
  };
}
...
const STATUS_CONFIG: Record<QuotationResponseStatus, StatusConfig> = {
  SENT: {
    label: "Sent",
    className:
...
const QuotationResponses: FC = () => {
  const [responses, setResponses] = useState<readonly QuotationResponse[]>([]);
    ...
    const loadResponses = async (): Promise<void> => {
      ...
      try {
        ...
        const mapped = result.data
          .map(mapQuotationToResponse)
          .filter((item): item is QuotationResponse => item !== null);

        setResponses(mapped);

        if (activeFilter === "ALL" && search.trim().length === 0) {
  ...
  const filteredResponses = useMemo(() => {
    ...
    return responses.filter((item) => {
      const matchesSearch =
        ...
        item.quotationNumber.toLowerCase().includes(normalizedSearch);
  ...
  const acceptedCount = allResponses.filter(
    (item) => item.status === "ACCEPTED",
  ).length;

  const rejectedCount = allResponses.filter(
    (item) => item.status === "REJECTED",
  ).length;

  const pendingCount = allResponses.filter(
              ...
              <table className="w-full min-w-[900px] border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/60">
                    ...
                    <TableHeader>Quotation Number</TableHeader>
                      ...
                      <tr>
                        <td colSpan={6} className="px-5 py-16 text-center">
                          <div className="mx-auto max-w-md">
                            ...
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                              No records match the current search or status
                              filter.
                            </p>
                          </div>
                        </td>
...
const QuotationResponseRow: FC<QuotationResponseRowProps> = ({
  response,
  onView,
}) => {
  ...
  return (
    <tr className="border-b border-slate-100 transition hover:bg-slate-50 last:border-0 dark:border-slate-800 dark:hover:bg-slate-800/50">
      ...
      <td className="px-5 py-4">
        <span className="whitespace-nowrap text-sm text-slate-600 dark:text-slate-400">
          {response.respondedAt.length > 0 ? response.respondedAt : "—"}
        </span>
      </td>
...
const QuotationResponseModal: FC<QuotationResponseModalProps> = ({
  response,
  onClose,
}) => {
          ...
          <div className="space-y-5 p-4 sm:p-6 md:p-7">
            ...
            >
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <DetailItem label="Company Name" value={response.companyName} />
            ...
            >
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <DetailItem label="Quotation Sent" value={response.sentAt} />
            ...
            {response.status === "SENT" && (
              <ResponseNotice
                ...
                title="Sent"
              >
                This quotation has been sent to the company administrator.
              </ResponseNotice>
            )}
            ...
            {response.status === "EXPIRED" && (
              ...
              >
                This quotation has expired without an active response.
              </ResponseNotice>
            )}
          </div>
        </div>
...
const ModalSection: FC<ModalSectionProps> = ({ title, icon, children }) => (
...
const TableHeader: FC<TableHeaderProps> = ({ children, align = "left" }) => (
  <th
...
interface FilterButtonProps {
  ...
  readonly onClick: (value: ResponseFilter) => void;
}
...
const SummaryCard: FC<SummaryCardProps> = ({ label, value, icon }) => (
