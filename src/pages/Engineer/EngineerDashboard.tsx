...
import { engineerComponentService } from "../../services/engineerComponentService";
...
const initialTasks: Task[] = [
  {
    ...
    dueDate: "15 May 2026, 06:30 PM",
    location: "Khumani Mine - Pit A",
    description:
      "Engine temperature is above the safe range. Inspect coolant level, radiator, and fan belt before machine operation.",
  },
  {
    id: "TSK-002",
    ...
    status: "Pending",
    assignedDate: "15 May 2026",
    dueDate: "15 May 2026, 07:00 PM",
    location: "Sishen Mine - Workshop Bay 2",
    description:
      "Hydraulic pressure dropped below threshold. Check hydraulic oil level, leakage, pump condition, and hose connections.",
  },
  {
    id: "TSK-003",
    machine: "CAT 740B",
    ...
    status: "In Progress",
    assignedDate: "15 May 2026",
    dueDate: "16 May 2026, 10:00 AM",
    location: "Kolomela Mine - Loading Area",
    description:
      "Tyre pressure is outside safe operating range. Inspect tyre damage and adjust pressure before next shift.",
  },
  {
    id: "TSK-004",
    machine: "Liebherr T 264",
    issue: "Suspension vibration check",
    priority: "Low",
    status: "Completed",
    assignedDate: "14 May 2026",
    dueDate: "15 May 2026, 12:00 PM",
    location: "Kathu Site - Maintenance Yard",
    description:
      "Routine suspension vibration inspection completed. No major fault detected.",
  },
];
...
const alerts: AlertItem[] = [
  {
    ...
    location: "Khumani Mine - Pit A",
    recommendation:
      "Stop machine safely and inspect cooling system immediately.",
  },
  {
    id: "ALT-002",
    machine: "Komatsu HD785",
    ...
    location: "Sishen Mine - Workshop Bay 2",
    recommendation:
      "Check hydraulic pressure reading and inspect possible leakage.",
  },
  {
    id: "ALT-003",
    machine: "CAT 740B",
    ...
    location: "Kolomela Mine - Loading Area",
    recommendation:
      "Inspect tyre pressure and avoid operating under heavy load.",
  },
];
...
export default function EngineerDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const dbLogs = Array.isArray(logsRes) ? logsRes : (logsRes.data || logsRes.logs || []);
      ...
      // Filter tasks assigned to currently logged in engineer
      const assignedLogs = dbLogs.filter((log: any) => {
        ...
        if (userEmail === "sefserferg@gmail.com" && (techName.includes("priya") || techName.includes("kumari"))) return true;
        if (userEmail === "shdbha@gmail.com" && (techName.includes("rt45t45") || techName.includes("54t45t45"))) return true;
        return false;
      });
      ...
      // Create a set of assigned machine names (lowercased) to only show assigned machines
      const assignedMachineNames = new Set(
        mappedTasks.map((t: any) => t.machine.toLowerCase().trim())
      );

      // Filter machines to ONLY show those that are assigned to this engineer
      const assignedMachinesOnly = machines.filter((m: any) => 
        assignedMachineNames.has(m.name.toLowerCase().trim())
      );

      const mappedMachineHealth = assignedMachinesOnly.map((m: any) => {
        const engineComp = m.components.find((c: any) => c.name === "Engine");
        const hydraulicComp = m.components.find((c: any) => c.name === "Hydraulics");

        return {
          id: m.machineId || m.id,
          machine: m.name,
          health: m.overallHealth,
          location: m.location,
          runtime: "8.4 hrs",
          engineTemp: engineComp?.readingValue || "82°C",
          hydraulicPressure: hydraulicComp?.readingValue || "210 bar",
        };
      });
      setMachineHealthState(mappedMachineHealth);

      const mappedAlerts: AlertItem[] = [];
      let alertCount = 1;
      assignedMachinesOnly.forEach((m: any) => {
        m.components.forEach((c: any) => {
          if (c.status === "Critical" || c.status === "Warning") {
            // Calculate dynamic relative time from component.updatedAt
            ...
            mappedAlerts.push({
              id: `ALT-00${alertCount++}`,
              machine: m.name,
              issue: `${c.name} ${c.issue.toLowerCase()}`,
  ...
  const stats = useMemo(
    () => [
      ...
      },
      {
        ...
        type: "maintenance" as ModalType,
      },
      {
        title: "Critical Alerts",
        value: String(
          alertsState.filter((alert) => alert.severity === "Critical").length,
        ),
        icon: <ShieldAlert size={24} strokeWidth={2.4} />,
        color:
          "border-red-200 bg-red-50 text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300",
        type: "alerts" as ModalType,
      },
      {
        title: "Assigned Machines",
        value: String(machineHealthState.length),
    ...
    [tasks, alertsState, machineHealthState],
  );
  ...
  const confirmTaskStatusChange = async () => {
    ...
    try {
      setIsLoading(true);
      await maintenanceService.updateLog((confirmTask as any).realId || confirmTask.id, {
        status: dbStatus
      });
      toast.success(
    ...
    <div className="min-h-screen bg-slate-100 p-4 font-sans text-slate-900 dark:bg-[#07111f] sm:p-6 lg:p-8">
      <div className="mx-auto max-w-[1500px] space-y-6">
            ...
            <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-300">
                  Live task monitoring, machine health prediction, and
                  maintenance response overview.
                </p>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
            {stats.map((item) => (
              <button
                ...
                className="group rounded-xl border border-slate-200 bg-slate-50 p-5 text-left transition hover:-translate-y-0.5 hover:border-blue-300 hover:bg-white hover:shadow-lg dark:border-slate-800 dark:bg-[#101f33] dark:hover:border-blue-500/50 dark:hover:bg-[#12243b]"
              >
                <div className="flex items-center justify-between gap-4">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-lg border ${item.color}`}
                  >
                    {item.icon}
                  </div>
        ...
        {tasks.filter(t => t.status !== "Completed").length > 0 && (() => {
          ...
          const tasksWithDays = openTasks.map(t => {
          ...
          return (
            <div className={`overflow-hidden rounded-2xl border p-5 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm transition-all hover:shadow-md ${
              isOverdue 
              ...
              <div className="flex items-center gap-4">
                <div className={`p-3 rounded-xl ${
                  isOverdue ? "bg-red-500/20 text-red-600 dark:text-red-300" : isDueToday ? "bg-amber-500/20 text-amber-600 dark:text-amber-300" : "bg-blue-500/20 text-blue-600 dark:text-blue-300"
                }`}>
                  <Clock size={24} className="animate-pulse" />
                </div>
                <div>
                  <h4 className="font-extrabold text-sm uppercase tracking-wider">Urgent Task Deadline Tracker</h4>
                  <p className="text-xs font-semibold mt-1 opacity-90">
                    Task <span className="font-bold">{nearestTask.id}</span> ({nearestTask.machine} - {nearestTask.issue}) is 
                    {isOverdue 
                      ? ` Overdue by ${Math.abs(nearestTask.daysRemaining)} days!` 
                      : isDueToday 
                      ? " Due Today!" 
                      : ` due in ${nearestTask.daysRemaining} days (${nearestTask.due}).`}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className={`px-4 py-1.5 rounded-full text-xs font-extrabold uppercase tracking-widest border ${
                  ...
                  {isOverdue ? "Overdue" : isDueToday ? "Due Today" : `${nearestTask.daysRemaining} Days Left`}
                </span>
              </div>
            </div>
          );
                  ...
                  {isLoading ? (
                    <tr>
                      <td colSpan={7} className="px-3 py-12 text-center">
                        <Loader2 size={36} className="animate-spin mx-auto text-blue-500" />
                        <p className="mt-3 text-xs font-bold text-slate-500 dark:text-slate-400">Loading assigned tasks...</p>
                      </td>
                    </tr>
                  ) : tasks.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-3 py-12 text-center text-sm font-bold text-slate-500 dark:text-slate-400">
              ...
              <div className="flex items-center justify-between border-b border-slate-200 bg-slate-950 p-5 dark:border-slate-800">
                <div>
                  <h2 className="text-xl font-extrabold tracking-tight text-white">
                    {activeModal === "tasks" && "All Engineer Tasks"}
