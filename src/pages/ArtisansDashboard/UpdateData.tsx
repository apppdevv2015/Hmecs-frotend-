...
export default function OperatorMachineDashboard() {
  ...
  const fetchAssignedMachine = useCallback(async () => {
    try {
      ...
      const assignedIds = await machineAssignmentService.getAssignedMachines(
        currentUser.id,
      );

      const filteredMachines = machines.filter((m) =>
        assignedIds.includes(m.machineId),
      );

      const convertedMachines = filteredMachines.map(convertFleetMachine);
